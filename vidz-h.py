...
class RamLoopPlayer(QObject):
    ...
    def start_loop(self, start_ms, end_ms):
        if not self.audio_data: return
        self.loop_start_bytes = int((start_ms / 1000.0) * self.byte_rate)
        self.loop_start_bytes -= self.loop_start_bytes % 4 
        self.loop_end_bytes = int((end_ms / 1000.0) * self.byte_rate)
        self.loop_end_bytes -= self.loop_end_bytes % 4
        ...
        loop_slice = self.audio_data[self.loop_start_bytes:self.loop_end_bytes]
        self.source_device.setData(loop_slice)
...
class AudioAnalysisWorker(QThread):
    ...
    def run(self):
        try:
            ...
            audio_vis = audio_vis.set_channels(1).set_frame_rate(22050)
            ...
            if not self.isInterruptionRequested():
                self.finished.emit(self.key, pixmap, bpm, duration_ms, audio_full, samples, 22050, self.gen_id)
            del audio_vis, samples, samples_float, vis_samples
            gc.collect()
        except:
            if not self.isInterruptionRequested():
...
class VJDeck:
    ...
    def start_ram_loop(self, start, end):
        self.loop_start_ms = int(start)
        self.audio.setMuted(True)
        self.ram_player.start_loop(start, end)
...
class LooperApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("VJ Looper v61 (Safety Fix)")
        self.resize(600, 950)
    ...
    def handle_beatjump(self, beats):
        try:
            ...
            if deck and deck.has_media():
                ...
                new_pos = int(current_pos + ms) # Explicit cast
    ...
    def eventFilter(self, source, event):
        try:
            if event.type() == QEvent.Type.KeyPress and not event.isAutoRepeat():
                key = event.key()
                text = event.text().lower() # Caps lock safety
                ...
                if key == Qt.Key.Key_Space: self.start_stutter(); return True
                ...
                if event.key() == Qt.Key.Key_Space: self.stop_stutter(); return True
    ...
    def assign_to_deck(self, deck, key):
        ...
        start_pos = self.find_nearest_zero_crossing(path, start_pos)
    ...
    def start_stutter(self):
        ...
        if deck and deck.has_media() and deck.ram_player.audio_data:
            ...
            end = self.find_nearest_zero_crossing(path, pos + self.current_loop_speed)
            deck.start_ram_loop(start, end)
    ...
    def on_deck_a_pos(self, pos):
        ...
        if path and path in self.manual_loops and not self.is_stuttering:
            ...
            if loop['active'] and pos >= loop['end']: 
                seek_pos = self.find_nearest_zero_crossing(path, loop['start'])
                self.deck_a.seek(seek_pos)
